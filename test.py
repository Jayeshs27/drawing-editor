...
class DrawingApp:
    ...
    def find_shape_path(self, start_x, start_y, end_x, end_y,path=[]):
        for i,item in enumerate(self.shapes):
            if self.shapes[i].start_x == start_x and self.shapes[i].start_y == start_y and self.shapes[i].end_x == end_x and self.shapes[i].end_y == end_y:
                return path + [i]
            elif isinstance(item, list):
                sub_path = self.find_shape_path(start_x, start_y, end_x, end_y, item, path + [i])
                if sub_path:
                    return sub_path
        return None

    # def print_nested_structure(li, indices):
    #     current = li.copy()
    #     for index in indices:
    #         current = current[index]
    #     print("Shape" + "".join(f"[{i}]" for i in indices) + ":", current)
    #     current += 500
        # print(li)

    def update_shape_vertices_given_path(self, indices, dx, dy):
        curr_shapes = self.shapes.copy()
        for index in indices:
            curr_shapes = curr_shapes[index]
        curr_shapes.start_x += dx
        curr_shapes.start_y += dy
        curr_shapes.end_x += dx
        curr_shapes.end_y += dy
                        ...
                        for j in range(len(flattened_list)):
                            if flattened_list[j].intersect(x1, y1, x2, y2):
                                for elem in flattened_list:
                                    ...
                                    found_path = self.find_shape_path(flattened_list[j].start_x,flattened_list[j].start_y,flattened_list[j].end_x,flattened_list[j].end_y)
                                    self.update_shape_vertices_given_path(found_path,dx,dy)
                                break

                    elif self.shapes[i].intersect(x1, y1, x2, y2):
                        self.canvas.move(self.shapes[i].shape, dx, dy)
                        found_path = self.find_shape_path(self.shapes[i].start_x,self.shapes[i].start_y,self.shapes[i].end_x,self.shapes[i].end_y)
                        print("PAATH:  ",found_path)
                        self.update_shape_vertices_given_path(found_path,dx,dy)
